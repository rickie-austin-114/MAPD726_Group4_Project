...
const ViewTransportScreen = ({ route, navigation }) => {
  ...
  const { tour, ticketPrice } = route.params;
    ...
    >
      ...
      >
        <ScrollView>
          ...
          <Text className="text-gray-700 ml-4">Price: {ticketPrice}</Text>
          ...
          <StripeProvider publishableKey="pk_test_51QuLVCPlUnLIZAQCnwrRbSpCJhgJZsH1PLPQEh9Jt9YUlJauxShMIQbxNKdKYmRkSP83OSsJeZQdsDwrK5IYwjvi00d0lp5KXm">
            <PaymentScreen name={tour.name} description={`Date: ${moment(startDate).format("MMMM D, YYYY")} - ${moment(endDate).format("MMMM D, YYYY")}`} image={tour.image} amount={ticketPrice} />
          </StripeProvider>
